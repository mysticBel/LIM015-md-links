...
const { totalLinks, uniqueLinks, brokenLinks, helpMsg, helloMsgValidate,helloMsgStats, helloMsgValidateAndStats,footerMsg, msgErrorRoute, msgNotLinks ,commandInvalidMsg } = require('./cli-stats.js');
...
if(option.length === 1) {
    ...
    if(validate && stats) {
        ...
        .then(res => {
            console.table(chalk.blueBright(helloMsgValidateAndStats))
            ...
            console.table(chalk.blueBright(footerMsg))})
        .catch(()=>console.log(chalk.magentaBright.bold(msgErrorRoute)))
    }else if (validate) {
        mdLinks(userPath, {validate:true})
        .then(res => {
        ...
        mdLinks(userPath, {validate:true})
        .then(res => {
            console.table(chalk.green(helloMsgStats))
            console.table(chalk.cyanBright(totalLinks(res)))
            console.table(chalk.yellowBright(uniqueLinks(res)))
       ...
       mdLinks(userPath, { validate:true })
        .then( console.log(chalk.red(commandInvalidMsg)),
               console.log(chalk.cyanBright(helpMsg)) )
        .catch(()=>console.log(chalk.magentaBright.bold(msgErrorRoute)))
    }
}